...
## Директория Транзакция.
### Модуль transaction_filter.py:
    Этот файл будет содержать функции для фильтрации транзакций.
### Модуль transaction_statistics.py
    Этот файл будет содержать функции для работы с данными о транзакциях.
### Модуль main.py
    Это основной файл приложения, который связывает все функции и управляет пользовательским вводом.

Функции фильтрации находятся в transaction_filter.py и используют регулярные выражения для поиска по описанию и подсчета операций по категориям.
Функции загрузки данных находятся в transaction_statistics.py и обрабатывают загрузку данных из различных форматов файлов.
Функция main в main.py управляет пользовательским вводом и связывает все функции между собой, обеспечивая необходимую логику для работы с транзакциями.
